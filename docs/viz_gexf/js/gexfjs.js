    ...
    function displayNode(_nodeIndex, _recentre) {
        ...
        if (_nodeIndex != -1) {
            ...
            var attribute_key_dict = {
                ...
                "d8":"Deaths A",
                "d9":"Deaths B",
                "d10":"Deaths Civilians",
                "d11":"Deaths Total",
                "degree":"Degree",
                "weighted degree":"Average Distance"
            };
            var type_of_violence_dict = {
                "1":"State Based",
                "2":"Non State",
                "3":"One Sided"
            };    
            ...
            for (var i = 0, l = _d.a.length; i < l; i++) {
                var attr = _d.a[reorder_dict[i]];
                var _li = $("<li>");
                var attrkey = GexfJS.graph.attributes[attr[0]];
                //$("<b>").text(strLang(attrkey) + ": ").appendTo(_li);
                $("<b>").text(attribute_key_dict[attrkey] + ": ").appendTo(_li);
                //$("<b>").text(attribute_key_dictr[attrkey] + ": ").appendTo(_li);
                if (attrkey === 'image') {
                    $('<br>').appendTo(_li);
                    $('<img>').attr("src", attr[1]).appendTo(_li).addClass("attrimg");
                } else if (attribute_key_dict[attrkey] === "Type of violence"){
                    _li.append(replaceURLWithHyperlinks(type_of_violence_dict[attr[1]]));
